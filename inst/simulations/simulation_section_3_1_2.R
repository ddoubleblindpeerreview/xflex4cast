################################################################################
# Simulation Code: Section 3.1.2 – Second Scenario: Non-Stationarity in the Tail
#
# Description:
# ------------
# This simulation assesses the predictive performance of the following methods:
#     - Standard QGAM
#     - X-flexForecast (with covariate-dependent tail modeling)
#     - Extended Discrete Generalized Pareto (EDGP) (https://doi.org/10.1177/1471082X24126672)
#
# Scenario details:
# - Non-stationarity is introduced in the tail via a second predictor x2, affecting
#   the scale parameter σ(x2) = exp(β0 + β1 * x2), with β0 = -0.05 and β1 = 0.85.
# - Covariates x1 and x2 are sampled jointly from empirical data distributions.
# - x1: wind speed; x2: 90th quantile of 24-hour precipitation.
# - The discrete Gamma bulk distribution setup is kept unchanged from Scenario 1.
#
# Model specifications:
# - QGAM and EDGP use penalized splines with basis size k = 10.
# - X-flexForecast uses true model specification in the tail.
#
# Simulation setup:
# - n = 5000 samples, φ = 0.1, ξ = 0.3
# - 10 replications (reduced from 1000 for illustration)
# - Quantiles evaluated: c(0.05, 0.25, ..., 0.9999)
#
# Output:
# - Summary RMSE ± SD table comparing each method per quantile.
# - Results saved as RDS file for reproducibility and plotting.
#
...
saveRDS(summary_formatted,
        file = paste0("inst/simulations/summary_formatted_",n_replications,"_replications_simulatio_3_1_2.rds"))
